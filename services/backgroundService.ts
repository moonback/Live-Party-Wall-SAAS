...
/**
 * Upload une image de fond pour le chroma key (fond vert) dans le bucket Supabase `party-backgrounds` pour un événement.
 * Nécessite un utilisateur authentifié (admin).
 * @param eventId - ID de l'événement
 * @param file - Fichier image à uploader
 * @returns Promise résolue avec l'URL publique et le chemin du fichier
...
export async function uploadGreenScreenBackground(
  ...
  const filenameSafe = file.name.replace(/[^\w.\-]/g, '_');
  // Organiser par événement : party-backgrounds/{eventId}/green-screen-{timestamp}-{filename}
  const path = `${eventId}/green-screen-${Date.now()}-${Math.random().toString(36).slice(2)}-${filenameSafe}`;
