...
- 📸 **Upload instantané** - Photo/vidéo avec compression automatique
- 🎨 **Mode collage** - Assemblez jusqu'à 4 photos en une seule création
- 📷 **Photobooth interactif** - Filtres & cadres en temps réel avec caméra
- ❤️ **Likes & réactions** - 6 types d'émojis disponibles (❤️ 😂 😢 🔥 😮 👍)
- 🔍 **Recherche IA "Retrouve-moi"** - Reconnaissance faciale pour trouver vos photos
- 📥 **Téléchargement** - Export individuel ou ZIP groupé haute définition
...
- 📊 **Dashboard temps réel** - Statistiques live avec analytics détaillées
- 🎪 **Multi-événements** - Architecture SaaS complète (gestion de plusieurs événements)
- 👮 **Modération IA** - Filtrage automatique du contenu inapproprié (toujours actif)
- ⚙️ **Personnalisation** - Paramètres granulaires (fond, logo, vitesse, délais)
- 🖼️ **Mode projection** - Optimisé pour grand écran avec transitions fluides
- ⚔️ **Battles photos** - Créez des duels votés en direct avec résultats en temps réel
...
- 👥 **Gestion d'équipe** - Rôles et permissions (Owner, Organizer, Viewer)
- 🔑 **Système de licences** - Gestion complète avec validation automatique
...
| **Realtime** | Supabase Realtime | (via Supabase) |
| **Auth** | Supabase Auth | (via Supabase) |
| **IA** | Google Gemini | 3 Flash / 2.5 Flash |
| **Reconnaissance faciale** | Face-api.js | 0.22.2 |
| **Performance** | @tanstack/react-virtual | 3.13.15 |
| **Validation** | Zod | 4.3.5 |
| **Utilitaires** | JSZip, File Saver, QRCode React | 3.10, 2.0, 4.2 |
...
| `npm run electron:dev` | Mode développement Electron (desktop) |
| `npm run electron:build` | Build Electron uniquement |
| `npm run electron:pack` | Build + Package Electron (créer l'installer) |
...
- 🛡️ **Row Level Security (RLS)** - Politiques granulaires sur toutes les tables
