          ...
          <div className="absolute inset-0 w-full h-full bg-black z-10">
            ...
            <div className="absolute bottom-0 left-0 w-full p-4 sm:p-8 pb-4 sm:pb-6 flex items-center justify-center space-x-6 sm:space-x-12 bg-gradient-to-t from-black/90 via-black/80 to-transparent backdrop-blur-sm pointer-events-none sm:pointer-events-auto">
              ...
              <button
                ...
                className={`relative w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 sm:border-[6px] border-white/90 flex items-center justify-center bg-transparent transition-all shadow-lg hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-pink-400/80 pointer-events-auto group
                  ${(countdown !== null && mediaType === 'photo') ? 'opacity-40 scale-95 cursor-not-allowed' : 'hover:bg-white/10 hover:border-white active:scale-90'}
                `}
                aria-label={mediaType === 'video' ? (isRecording ? 'Arrêter l\'enregistrement' : 'Démarrer l\'enregistrement') : 'Prendre une photo'}
              >
                {/* Outer glow effect - pulse animé quand prêt */}
                ...
                {/* Glow effect intensifié au hover */}
                ...
                {/* Bouton principal */}
                <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full border-2 sm:border-4 border-transparent transition-all duration-300 shadow-lg ${
                  isRecording && mediaType === 'video'
                    ? 'bg-red-600 animate-pulse border-red-400/50'
                    : mediaType === 'video'
                    ? 'bg-gradient-to-br from-blue-500 via-purple-500 to-blue-600 group-hover:from-blue-400 group-hover:via-purple-400 group-hover:to-blue-500 group-hover:border-white/60 group-hover:scale-110 group-hover:shadow-xl group-hover:shadow-blue-500/50'
                    : countdown !== null
                    ? 'bg-gradient-to-br from-red-500 via-pink-500 to-red-600 animate-pulse border-red-400/50'
                    : 'bg-gradient-to-br from-red-500 via-pink-500 to-red-600 group-hover:from-red-400 group-hover:via-pink-400 group-hover:to-red-500 group-hover:border-white/60 group-hover:scale-110 group-hover:shadow-xl group-hover:shadow-pink-500/50'
                }`}>
                  {/* Inner shine avec effet de rotation subtil */}
                  <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white/30 via-white/10 to-transparent group-hover:from-white/40 group-hover:via-white/20 transition-all duration-300" />
                  ...
                  {/* Icône au centre pour plus de clarté */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    {mediaType === 'video' ? (
                      isRecording ? (
                        <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-sm bg-white shadow-lg" />
                      ) : (
                        <div className="w-0 h-0 border-l-[6px] sm:border-l-[8px] border-l-white border-t-[4px] sm:border-t-[6px] border-t-transparent border-b-[4px] sm:border-b-[6px] border-b-transparent ml-0.5 sm:ml-1 shadow-lg" />
                      )
                    ) : (
                      <CameraIcon className="w-5 h-5 sm:w-6 sm:h-6 text-white/90 drop-shadow-lg" strokeWidth={2.5} />
                    )}
                  </div>
                  
                  {/* Indicateur de pulse subtil quand prêt */}
                ...
                {/* Effet de lumière concentré au centre */}
                {!cameraError && countdown === null && (
