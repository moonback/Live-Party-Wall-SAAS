...
export const AftermovieCard: React.FC<AftermovieCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-gradient-to-br from-slate-900/60 to-slate-800/60 backdrop-blur-md rounded-xl overflow-hidden border border-slate-700/50 shadow-xl hover:border-indigo-500/50 hover:shadow-indigo-500/20 transition-all duration-300"
    >
      ...
      <div className="p-3">
        ...
        <div className="mt-3 pt-3 border-t border-slate-700/50 md:hidden">
          <button
            onClick={() => onDownload(aftermovie)}
            disabled={isDownloading}
            className="w-full px-3 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:from-slate-700 disabled:to-slate-700 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-50"
          >
            {isDownloading ? (
              <>
