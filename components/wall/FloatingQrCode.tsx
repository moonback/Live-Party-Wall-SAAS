...
const FloatingQrText: React.FC = () => (
  <div className="text-center mt-3 md:mt-4 relative z-10">
    <p className="text-slate-800 font-extrabold text-xs md:text-sm lg:text-base uppercase tracking-[0.2em] mb-1 group-hover:text-pink-600 transition-colors drop-shadow-sm">
      Envoyer
    </p>
    <p className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-pink-600 to-purple-600 font-extrabold text-[10px] md:text-xs lg:text-sm animate-gradient">
      une photo !
    </p>
  </div>
);
    ...
    return (
      <div className={getPositioningClasses(isKiosqueMode, show)}>
        <div className={getCardClasses(isKiosqueMode)}>
          {/* Glow effect */}
          <div className="absolute -inset-2 md:-inset-3 bg-gradient-to-r from-pink-500 via-purple-500 to-cyan-500 rounded-2xl md:rounded-3xl opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-500" />
          {/* Backdrop */}
          <div className="absolute -inset-1 md:-inset-2 bg-black/5 rounded-2xl md:rounded-3xl backdrop-blur-sm" />
