...
export const CaptureButton: React.FC<CaptureButtonProps> = ({
  ...
  const isDisabled = disabled || cameraError || countdown !== null;
  ...
  return (
    ...
    >
      ...
      {!cameraError && countdown === null && (
        <div className="absolute inset-0 rounded-full transition-all duration-500 md:hidden bg-pink-500/25 group-hover:bg-pink-500/40 blur-2xl animate-[pulse_2s_ease-in-out_infinite]" />
      )}
      
      {/* Glow effect intensifié au hover (masqué sur desktop) */}
      {!cameraError && countdown === null && (
        <div className="absolute inset-0 rounded-full transition-all duration-300 md:hidden bg-pink-500/30 group-hover:bg-pink-500/50 group-hover:scale-110 blur-xl" />
      )}
      ...
      {!cameraError && countdown === null && (
        <div className="absolute inset-0 rounded-full pointer-events-none md:hidden bg-pink-400/20 group-hover:bg-pink-400/30 blur-sm transition-all duration-300" 
        style={{ 
