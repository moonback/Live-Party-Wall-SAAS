...
export const ProjectionWall: React.FC<ProjectionWallProps> = ({
  ...
  useEffect(() => {
    ...
    const subscription = subscribeToNewPhotos(currentEvent.id, async (newPhoto: Photo) => {
      try {
  ...
  }, [transitionDuration, settings.ar_scene_enabled, currentEvent?.id]);
  ...
  if (displayedPhotos.length === 0) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-slate-950 via-purple-950/20 to-pink-950/20 flex items-center justify-center overflow-hidden">
        {/* Background Decor animé - plein écran */}
        <div className="absolute inset-0 pointer-events-none">
          <motion.div
            ...
            className="absolute top-0 right-0 w-full h-full bg-purple-900/20 rounded-full blur-[200px]"
          />
          <motion.div
            animate={{
              scale: [1.2, 1, 1.2],
            ...
            className="absolute bottom-0 left-0 w-full h-full bg-pink-900/20 rounded-full blur-[200px]"
          />
        </div>

        {/* Contenu plein écran avec grille */}
        <div className="relative z-10 w-full h-full flex items-center justify-center p-6 md:p-8 lg:p-12">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.4, ease: "easeOut" }}
            className="w-full h-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 lg:gap-12 items-center"
          >
            ...
            <div className="flex flex-col justify-center space-y-4 md:space-y-6">
              ...
              >
                <motion.div
                  ...
                  className="relative"
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-pink-500/30 to-purple-500/30 rounded-full blur-xl" />
                  <div className="relative bg-gradient-to-br from-pink-500/20 via-purple-500/20 to-indigo-500/20 p-4 rounded-full border-2 border-pink-500/40">
                    <Camera className="w-8 h-8 md:w-10 md:h-10 text-pink-400" />
                  </div>
                </motion.div>
                <div>
                  <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400">
              ...
              >
                ...
                />
                <span className="text-xs md:text-sm text-slate-400">En attente de photos...</span>
              </motion.div>

              {/* Message descriptif compact */}
              <motion.p
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.2, duration: 0.4 }}
                className="text-slate-300 text-sm md:text-base lg:text-lg leading-relaxed"
              >
                Le mur est vide pour le moment. Scannez le QR code pour partager vos photos et commencer à remplir le mur !
              </motion.p>

              {/* Instructions compactes en ligne */}
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-2 border border-slate-700/50">
                  ...
                  <p className="text-xs text-slate-400">Ouvrir appareil</p>
                </div>
                <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-2 border border-slate-700/50">
                  <span className="text-lg">📷</span>
                  <p className="text-xs text-slate-400">Scanner QR</p>
                </div>
                <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-2 border border-slate-700/50">
                  <span className="text-lg">✨</span>
                  <p className="text-xs text-slate-400">Partager</p>
                </div>
              </motion.div>
              ...
              {onBack && (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={onBack}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 rounded-lg transition-all duration-300 text-white text-sm font-medium shadow-lg shadow-pink-900/30 w-fit"
                >
            ...
            {/* Colonne droite - QR Code centré et grand */}
            <motion.div
            ...
            >
              <div className="relative group">
                ...
                <motion.div
                  ...
                  className="absolute -inset-6 bg-gradient-to-r from-pink-500 via-purple-500 to-cyan-500 rounded-3xl blur-2xl"
                />
                
                {/* QR Code Container - plus grand pour remplir l'écran */}
                <motion.div
                  whileHover={{ scale: 1.03 }}
                  className="relative bg-gradient-to-br from-white via-white to-gray-50 p-4 md:p-6 rounded-3xl shadow-2xl border-2 border-white/50"
                >
                  <div className="bg-white p-3 md:p-4 rounded-2xl shadow-inner relative">
                    <QRCodeCanvas
                      value={uploadUrl}
                      size={280}
                      level="H"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <motion.div
                        ...
                        className="bg-white rounded-full p-2 md:p-3 shadow-xl border-2 border-pink-200"
                      >
                  ...
                  <div className="text-center mt-3 md:mt-4">
                    <p className="text-slate-900 font-extrabold text-xs md:text-sm uppercase tracking-wider mb-1 group-hover:text-pink-600 transition-colors">
                      Envoyer
                    </p>
                    <p className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-pink-600 to-purple-600 font-extrabold text-sm md:text-base">
                      une photo !
                    </p>
                  </div>
                  ...
                  <motion.div
                    ...
                    className="absolute top-2 left-2 w-3 h-3 border-t-2 border-l-2 border-pink-400 rounded-tl-lg"
                  />
                  <motion.div
                    animate={{ opacity: [0.4, 0.8, 0.4] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
                    className="absolute top-2 right-2 w-3 h-3 border-t-2 border-r-2 border-pink-400 rounded-tr-lg"
                  />
                  <motion.div
                    animate={{ opacity: [0.4, 0.8, 0.4] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                    className="absolute bottom-2 left-2 w-3 h-3 border-b-2 border-l-2 border-pink-400 rounded-bl-lg"
                  />
                  <motion.div
                    animate={{ opacity: [0.4, 0.8, 0.4] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", delay: 1.5 }}
                    className="absolute bottom-2 right-2 w-3 h-3 border-b-2 border-r-2 border-pink-400 rounded-br-lg"
                  />
                </motion.div>
              </div>
            </motion.div>
